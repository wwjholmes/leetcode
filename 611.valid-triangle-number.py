    ...
    def triangleNumber(self, nums: List[int]) -> int:
        ...
        for k in range(m - 1, 1, -1):
            for j in range(k - 1, 0, -1):
                find_solution = False
                target = sorted_nums[k] - sorted_nums[j]
                index = self.findIndex(sorted_nums, 0, j - 1, target)
                if index != -1:
                    # print(sorted_nums[index], sorted_nums[j], sorted_nums[k])
                    # print('count:', j - index)
                    count += j - index

        return count

    def findIndex(self, sorted_nums: List[int], start: int, end: int, target: int):
        left = start
        right = end
        while left < right:
            mid = (left + right) // 2
