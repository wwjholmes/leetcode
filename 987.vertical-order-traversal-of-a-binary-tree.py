...
class Solution:
    def verticalTraversal(self, root: TreeNode) -> List[List[int]]:
        ...
        def traversal(root: TreeNode, row: int, col:int):
            if root == None:
                return
            if row in h[col]:
                h[col][row].append(root.val)
            else:
                h[col][row] = [root.val]
        ...
        h = defaultdict(dict)
        # traverse all nodes in binary tree and add then to h
        traversal(root, 0, 0)
        verticals = []
        for key1 in sorted(h.keys()):
            h_rows = h[key1]
            nums_in_column = []
            for key2 in sorted(h_rows.keys()):
                nums = sorted(h_rows[key2])
                nums_in_column += nums
            verticals.append(nums_in_column)
