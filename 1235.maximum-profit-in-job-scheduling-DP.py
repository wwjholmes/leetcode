...
class Solution:
    def jobScheduling(self, startTime: List[int], endTime: List[int], profit: List[int]) -> int:
        ...
        jobs.sort()
        dp = [0 for i in range(len(profit) + 1)]
        for i in range(1, len(profit) + 1):
            # case 1, current job selected
            idx = bisect_right(jobs, (jobs[i-1][1], jobs[i-1][1], 0), hi=i)
            max_profit_with = jobs[i-1][2] + dp[idx]

            # case 2, curretn job not selected
            max_profit_without = dp[i-1]

            dp[i] = max(max_profit_with, max_profit_without)
