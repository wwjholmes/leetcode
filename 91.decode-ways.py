...
class Solution:
    def numDecodings(self, s: str) -> int:
        # f(n) = f(n-1) + f(n-2) // n >= 2
        listOfCodes = [str(i) for i in range(1,27)]
        codes = dict.fromkeys(listOfCodes, True)

        def isValidEncoding(c: str) -> int:
            return 1 if c in codes else 0

        f1 = isValidEncoding(s[0])
        if len(s) == 1 or f1 == 0:
            return f1
        ...
        for i in range(1, len(s)):
            current = prev * \
                isValidEncoding(s[i]) + prev_prev * isValidEncoding(s[i-1:i+1])
            if current == 0:
                return 0
            prev_prev = prev
            prev = current
